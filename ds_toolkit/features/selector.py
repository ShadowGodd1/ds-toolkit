...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def _stage_rfecv(self, X: pd.DataFrame, y: pd.Series, cols: List[str]) -> List[str]:
        ...
        dropped = [c for c, m in zip(cols, mask) if not m]
        for c in dropped:
            self._drop_log.append({"feature": c, "stage": "rfecv", "reason": "eliminated by RFECV"})
        return kept

    def _stage_shap(self, X: pd.DataFrame, y: pd.Series, cols: List[str]) -> List[str]:
        try:
            import shap
        except ImportError as exc:
            raise ImportError("SHAP selection requires shap: pip install shap") from exc
    ...
    def _get_estimator(self):
        ...
        if self.task == "clf":
            ...
            return RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=self.n_jobs)
        from sklearn.ensemble import RandomForestRegressor

        return RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=self.n_jobs)
