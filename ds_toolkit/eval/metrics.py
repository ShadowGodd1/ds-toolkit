...
class MetricsReport:
    ...
    def _clf_metrics(
        ...
        rows = [
            ...
            (
                "precision",
                round(
                    float(precision_score(y_true, y_pred, average=self.average, zero_division=0)), 6
                ),
            ),
            (
                "recall",
                round(
                    float(recall_score(y_true, y_pred, average=self.average, zero_division=0)), 6
                ),
            ),
            (
                "f1",
                round(float(f1_score(y_true, y_pred, average=self.average, zero_division=0)), 6),
            ),
        ...
        if y_proba is not None:
            try:
                if is_binary:
                    ...
                    rows.append(
                        ("pr_auc", round(float(average_precision_score(y_true, proba_col)), 6))
                    )
                else:
                    rows.append(
                        (
    ...
    def _reg_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> list:
        n = len(y_true)
        ...
        if not np.any(y_true == 0):
            rows.insert(
                3, ("mape", round(float(mean_absolute_percentage_error(y_true, y_pred)), 6))
            )
