...
class ColumnSchema:
    ...
    def __init__(self, col_name: str, definition: Dict[str, Any]) -> None:
        ...
        if unknown:
            raise ValueError(f"Column '{col_name}' has unknown schema keys: {sorted(unknown)}")
...
class SchemaValidator:
    ...
    def check(
        ...
        parsed = {col: ColumnSchema(col, defn) for col, defn in schema.items()}
        ...
        violations_df = (
            ...
            if violations
            else pd.DataFrame(columns=["column", "check", "detail"])
        )
    ...
    def _check_column(self, series: pd.Series, schema: ColumnSchema) -> List[Dict[str, str]]:
        violations: List[Dict[str, str]] = []
        col = schema.col_name
        ...
        if not dtype_ok:
            violations.append(
                self._violation(
                    col, "dtype", f"Expected dtype '{schema.dtype}', got '{series.dtype}'"
                )
            )
        ...
        if not schema.nullable and n_null > 0:
            violations.append(
                self._violation(
                    col, "nullable", f"{n_null} null value(s) found; column is marked non-nullable"
                )
            )
        ...
        if schema.unique:
            ...
            if n_dup > 0:
                violations.append(
                    self._violation(
                        col, "unique", f"{n_dup} duplicate value(s) found; column must be unique"
                    )
                )
            ...
            if pd.api.types.is_numeric_dtype(series):
                if schema.min is not None:
                    ...
                    if n_below:
                        violations.append(
                            self._violation(
                                col, "min", f"{n_below} value(s) below minimum {schema.min}"
                            )
                        )
                if schema.max is not None:
                    ...
                    if n_above:
                        violations.append(
                            self._violation(
                                col, "max", f"{n_above} value(s) above maximum {schema.max}"
                            )
                        )
            else:
                violations.append(
                    self._violation(
                        col, "range_inapplicable", "min/max constraints require a numeric column"
                    )
                )
