...
class OutlierDetector(BaseEstimator):
    ...
    def _iqr_mask(self, series: pd.Series) -> Tuple[pd.Series, float, float]:
        clean = series.dropna()
    ...
    def _zscore_mask(self, series: pd.Series) -> Tuple[pd.Series, float, float]:
        clean = series.dropna()
    ...
    def _isoforest_mask(self, df: pd.DataFrame, cols: list) -> pd.Series:
        try:
    ...
    def _lof_mask(self, df: pd.DataFrame, cols: list) -> pd.Series:
        try:
