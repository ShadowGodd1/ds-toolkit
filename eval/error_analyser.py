    ...
    def _error_figures(
        ...
        try:
            ...
            ax.axvline(threshold, color="red", linestyle="--",
                       linewidth=1.2, label=f"Worst {self.n_worst:.0%} threshold")
            ax.set_xlabel("Error")
            ax.set_ylabel("Count")
        ...
        try:
            ...
            if numeric_cols:
                ...
                width = 0.35
                ax.barh(x + width / 2, rest_means.values, width,
                        label="Rest", color="#4C72B0", alpha=0.8)
                ax.barh(x - width / 2, worst_means.values, width,
                        label="Worst", color="#DD4444", alpha=0.8)
                ax.set_yticks(x)
