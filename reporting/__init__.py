...
class HTMLExporter:
    ...
    def _build_full_html(self, title, cv, ev, shap, diag) -> str:
        ...
        if ev is not None:
            body_parts.append(
                f"<h2>Metrics</h2>{ev.metrics_df.to_html(border=0)}"
            )
...
class ModelCard(DisplayMixin):
    ...
    def _repr_html_(self) -> str:
        ...
        # Overview
        sections.append(section("Model Overview", f"""
        ...
        if self.metrics:
            ...
            sections.append(section("Performance Metrics", f"""
        ...
        if self.cv_summary is not None:
            sections.append(section(
                ...
                self.cv_summary.to_html(border=0, index=False,
        ...
        if self.top_features:
            ...
            sections.append(section("Top Features (SHAP)", f"""
            <table style="border-collapse:collapse;font-size:12px">
        ...
        # Limitations
        if self.limitations:
            items = "".join(f"<li>{l}</li>" for l in self.limitations)
        ...
        if self.experiment_info:
            ...
            sections.append(section("Experiment Log", f"""
    ...
    def to_md(self) -> str:
        ...
        lines = [f"# Model Card — {self.model_name}", "", f"**Task:** {self.task.upper()}",
...
def generate_model_card(
    ...
    task = (cv_results.task if cv_results else
            eval_results.task if eval_results else "unknown")
    ...
    if error_report is not None:
        flagged = error_report.segments_df[error_report.segments_df["flagged"] == True]
